...
class TreapNodeRMQ(TreapNodeDS):        
    ...
    def update_data(self):
        super().update_data()
        if self.left is not None:
            self.left_child = self.left.left_child + self.left.right_child + 1
        else:
            self.left_child = 0
        if self.right is not None:
            self.right_child = self.right.left_child + self.right.right_child + 1
        else:
            self.right_child = 0

    def find_k(self, k):
        left = self.count + self.left_child - 1
        if left == k - 1:
            return self
        if left > k - 1 and self.left is not None:
            return self.left.find_k(k)
        elif left < k - 1 and self.right is not None:
            return self.right.find_k(k - left - 1)
        return self
...
for i in arr:
    node = t.find(i)
    if node is None:
        t.insert(i, random.randint(1, 20))
    else:
        node.count += 1
...
for question in questions:
    if question > 0:
        node = t.find(question)
        if node is None:
            t.insert(float(question), random.randint(1, 20))
            t.root.update_data()
        else:
            node.count += 1
    else:
        node = t.root.find_k(abs(question))
        if node.count > 1:
            node.count -= 1
        else:
            t.remove(node.key)
